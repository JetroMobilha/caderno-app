...
class _TableEditToolbarState extends ConsumerState<TableEditToolbar> {
  ...
  void _handleExit() {
    debugPrint('✅ [TableToolbar] Finalizando intenção de manipular tabela.');
  ...
  Widget build(BuildContext context) {
    ...
    // 🚀 v4.8: Localização segura da tabela (Evitar Bad State: No element)
    TableObject? table;
    try {
      table = page.objects.whereType<TableObject>().firstWhere(
        (t) => toolState.selectedTableIds.contains(t.id) || t.id == toolState.activeTableId,
      );
    } catch (_) {
      // Se não encontrou pela ativa, tentamos pela primeira selecionada
      if (toolState.selectedTableIds.isNotEmpty) {
        try {
          table = page.objects.whereType<TableObject>().firstWhere(
            (t) => toolState.selectedTableIds.contains(t.id),
          );
        } catch (_) {}
      }
    }
    ...
    final cellCoords = toolState.activeTableCell?.split(',');
    final TableCellModel? activeCell = (table != null && cellCoords != null) 
        ? table.cells['${cellCoords[0]},${cellCoords[1]}'] 
        : null;

    final selectedCells = (table != null) 
        ? toolState.selectedTableCells.map((key) => table!.cells[key] ?? TableCellModel()).toList() 
        : <TableCellModel>[];
        ...
        children: [
          Row(
            ...
            _buildCustomIconButton(
              ...
              // 🚀 v6.5: Modo de Transformação Deliberado (Proteção de Toque)
              _buildCustomIconButton(
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      _buildCategoryTab(TableEditCategory.structure, Icons.grid_view_rounded, 'Estrutura'),
                      _buildCategoryTab(TableEditCategory.cell, Icons.edit_attributes_rounded, 'Célula'),
                      _buildCategoryTab(TableEditCategory.style, Icons.palette_rounded, 'Estilo'),
                      _buildCategoryTab(TableEditCategory.actions, Icons.layers_outlined, 'Ações'),
                    ],
                  ),
                ),
              ),
          ...
          Center(
            child: SingleChildScrollView(
              scrollDirection: Axis.horizontal,
              child: _buildActiveCategoryContent(context, table, activeCell, toolNotifier, docNotifier, page),
            ),
          ),
        ],
      ),
  ...
  Widget _buildActiveCategoryContent(BuildContext context, TableObject table, TableCellModel? activeCell, CanvasToolNotifier toolNotifier, CanvasDocumentNotifier docNotifier, LocalPage page) {
    ...
    switch (_activeCategory) {
          ...
          children: [
            _buildCustomIconButton(icon: Icons.table_rows_rounded, onTap: () {
              ...
              if (selectedKeys.isNotEmpty) {
                ...
                final coords = key.contains(':') ? key.split(':')[1] : key;
                insertAt = int.parse(coords.split(',')[0]) + 1;
              }
            ...
            _buildCustomIconButton(icon: Icons.view_column_rounded, onTap: () {
              ...
              if (selectedKeys.isNotEmpty) {
                final key = selectedKeys.first;
                final coords = key.contains(':') ? key.split(':')[1] : key;
                insertAt = int.parse(coords.split(',')[1]) + 1;
              }
              table.insertColumn(insertAt);
              docNotifier.updateObject(page, table);
            }, color: const Color(0xFF0F4C5C)),
            const VerticalDivider(width: 12),
            // 🚀 v6.2: Seleção Rápida de Linha/Coluna
            _buildCustomIconButton(icon: Icons.select_all_outlined, onTap: () {
              if (selectedKeys.isEmpty) return;
              final key = selectedKeys.first;
              final coords = key.contains(':') ? key.split(':')[1] : key;
              final row = int.parse(coords.split(',')[0]);
              
              final Set<String> keys = {};
              for (int c = 0; c < table!.cols; c++) { keys.add('${table.id}:$row,$c'); }
            ...
            _buildCustomIconButton(icon: Icons.view_week_outlined, onTap: () {
              ...
              final key = selectedKeys.first;
              final coords = key.contains(':') ? key.split(':')[1] : key;
              final col = int.parse(coords.split(',')[1]);
              
              final Set<String> keys = {};
              for (int r = 0; r < table!.rows; r++) { keys.add('${table.id}:$r,$col'); }
              toolNotifier.selectIds(tableCells: keys);
            }, color: Colors.blueAccent, size: 18),
            ...
            _buildCustomIconButton(icon: Icons.delete_sweep_rounded, onTap: () {
              if (table.rows > 1) {
                ...
                if (selectedKeys.isNotEmpty) {
                  final key = selectedKeys.first;
                  final coords = key.contains(':') ? key.split(':')[1] : key;
                  deleteAt = int.parse(coords.split(',')[0]);
                }
                table.deleteRow(deleteAt);
                toolNotifier.selectIds(tableCells: {}); // Limpar seleção para evitar erro
                docNotifier.updateObject(page, table);
              }
            ...
            _buildCustomIconButton(icon: Icons.view_week_rounded, onTap: () {
              if (table.cols > 1) {
                ...
                if (selectedKeys.isNotEmpty) {
                  final key = selectedKeys.first;
                  final coords = key.contains(':') ? key.split(':')[1] : key;
                  deleteAt = int.parse(coords.split(',')[1]);
                }
                table.deleteColumn(deleteAt);
      ...
      case TableEditCategory.cell:
        ...
        // Usamos a primeira selecionada como referência para o estado dos toggles
        final firstKey = selectedKeys.first;
        final refCoords = firstKey.contains(':') ? firstKey.split(':')[1] : firstKey;
        final refCell = table.cells[refCoords] ?? TableCellModel();
          ...
          children: [
            ...
            _buildFormatToggle(Icons.format_bold, refCell.style.bold, () {
              for (var key in selectedKeys) {
                ...
                cell.style.bold = !refCell.style.bold;
                table.cells[coords] = cell;
              }
              docNotifier.updateObject(page, table);
            }),
            _buildFormatToggle(Icons.format_italic, refCell.style.italic, () {
              for (var key in selectedKeys) {
                final coords = key.contains(':') ? key.split(':')[1] : key;
                final cell = table.cells[coords] ?? TableCellModel();
                cell.style.italic = !refCell.style.italic;
                table.cells[coords] = cell;
              }
              docNotifier.updateObject(page, table);
            }),
            ...
            _buildCustomIconButton(icon: Icons.format_color_fill_rounded, onTap: () async {
              ...
              if (hex != null) {
                for (var key in selectedKeys) {
                  final coords = key.contains(':') ? key.split(':')[1] : key;
                  final cell = table.cells[coords] ?? TableCellModel();
                  cell.style.backgroundColorHex = hex;
                  table.cells[coords] = cell;
                }
                docNotifier.updateObject(page, table);
              }
      ...
      case TableEditCategory.style:
        return Row(
          children: [
            _buildColorCircle(context, table.borderColor, (hex) {
            ...
            _buildCustomIconButton(icon: Icons.format_paint_rounded, onTap: () async {
              ...
              if (hex != null) {
                table.tableBackgroundColorHex = hex;
                docNotifier.updateObject(page, table);
              }
            }, color: table.tableBackgroundColorHex != null ? Color(int.parse(table.tableBackgroundColorHex!.replaceFirst('#', '0xFF'))) : Colors.black26),
            const VerticalDivider(width: 20),
            _buildFormatToggle(Icons.view_headline_rounded, table.showHeader, () {
              table.showHeader = !table.showHeader;
              docNotifier.updateObject(page, table);
            }),
          ],
        );
        ...
        return Row(
          children: [
            _buildCustomIconButton(
              ...
              onTap: selectedKeys.length > 1 ? () => _handleMerge(table, selectedKeys, docNotifier, page) : null, 
              color: selectedKeys.length > 1 ? Colors.blueAccent : Colors.black12
            ),
            _buildCustomIconButton(
              icon: Icons.call_split_rounded, 
              onTap: () => _handleSplit(table, selectedKeys, docNotifier, page), 
              ...
              onTap: () {
                ...
                for (int r = 0; r < table.rows; r++) {
                  for (int c = 0; c < table.cols; c++) {
                    allKeys.add('${table.id}:$r,$c');
                  }
                }
                ref.read(canvasToolProvider.notifier).selectIds(tableCells: allKeys);
              }, 
              color: const Color(0xFF0F4C5C)
            ),
          ],
  ...
  void _handleMerge(TableObject table, Set<String> selectedKeys, CanvasDocumentNotifier docNotifier, LocalPage page) {
    ...
    // Criar o span (Key: top-left, Value: "rows,cols")
    int rowSpan = (maxR - minR) + 1;
    int colSpan = (maxC - minC) + 1;
    
    final topLeftKey = '$minR,$minC';
    final selectionKey = '${table.id}:$topLeftKey';
    table.cellSpans[topLeftKey] = '$rowSpan,$colSpan';

    // Limpar os dados das outras células que agora estão "cobertas"
    for (int r = minR; r <= maxR; r++) {
      for (int c = minC; c <= maxC; c++) {
        if (r == minR && c == minC) continue;
        table.cells.remove('$r,$c');
      }
    }
  ...
  void _handleSplit(TableObject table, Set<String> selectedKeys, CanvasDocumentNotifier docNotifier, LocalPage page) {
    for (var key in selectedKeys) {
      final coords = key.contains(':') ? key.split(':')[1] : key;
      if (table.cellSpans.containsKey(coords)) {
        final span = table.cellSpans[coords]!.split(',');
        int rs = int.parse(span[0]);
        int cs = int.parse(span[1]);
        
        table.cellSpans.remove(coords);
        
        // Opcional: Recriar TableCellModel para as células que estavam "cobertas"
        final parts = coords.split(',');
        int startR = int.parse(parts[0]);
        int startC = int.parse(parts[1]);
        
        for (int r = startR; r < startR + rs; r++) {
          for (int c = startC; c < startC + cs; c++) {
            if (r == startR && c == startC) continue;
            table.cells['$r,$c'] = TableCellModel();
          }
        }
      }
    }
  ...
  Widget _buildCellTypeDropdown(TableObject table, Set<String> selectedKeys, CanvasDocumentNotifier docNotifier, LocalPage page) {
    final firstKey = selectedKeys.first;
    final refCoords = firstKey.contains(':') ? firstKey.split(':')[1] : firstKey;
    final refCell = table.cells[refCoords] ?? TableCellModel();
    ...
    return PopupMenuButton<TableCellType>(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Icon(_getCellTypeIcon(refCell.type), size: 14),
            const SizedBox(width: 6),
      ...
      onSelected: (type) {
        for (var key in selectedKeys) {
          ...
          final cell = table.cells[coords] ?? TableCellModel();
          cell.type = type;
          if (type == TableCellType.checkbox && cell.value.isEmpty) cell.value = 'false';
          table.cells[coords] = cell;
        }
        docNotifier.updateObject(page, table);
      },
  ...
  IconData _getCellTypeIcon(TableCellType type) {
    switch (type) {
      case TableCellType.number: return Icons.numbers_rounded;
      case TableCellType.date: return Icons.calendar_today_rounded;
      case TableCellType.checkbox: return Icons.check_box_rounded;
  ...
  Widget _buildColorCircle(BuildContext context, String initialColor, Function(String) onSelected, {String? label}) {
    return InkWell(
      ...
      child: Padding(
        ...
        child: CircleAvatar(
          radius: 11,
          backgroundColor: Color(int.parse(initialColor.replaceFirst('#', '0xFF'))),
