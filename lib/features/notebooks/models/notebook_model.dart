...
/// 🚀 v9.4: Modelo de Caderno Imutável.
class Notebook {
  final int? id;
  final int? serverId;
  ...
  final int pageCount;
  final NotebookConfiguration? configuration;
  ...
  Map<String, dynamic> toJson() {
    return {
      'id': id, 'server_id': serverId, 'client_id': clientId, 'subject_id': subjectId,
      'title': title, 'cover_type': coverType, 'color': color, 'cover_image': coverImage,
      'template_type': templateType, 'collaboration_mode': collaborationMode,
      'is_published': isPublished, 'price': price, 'description': description,
      'author_name': authorName, 'synced_with_cloud': syncedWithCloud, 'is_deleted': isDeleted,
      'updated_at': updatedAt, 'version': version, 'role': role,
      'alternative_title': alternativeTitle, 'sharing_type': sharingType, 'tags': tags,
      'is_archived': isArchived ? 1 : 0, 'is_favorite': isFavorite ? 1 : 0, 'origin': origin,
      'participants_preview': {'total': participantsTotal, 'list': participants.map((e) => e.toJson()).toList(), 'online_count': onlineCount},
      'last_updated_by_name': lastUpdatedByName, 'notifications_enabled': notificationsEnabled ? 1 : 0,
  ...
  factory Notebook.fromJson(Map<String, dynamic> json) {
    ...
    if (preview != null && preview is Map) {
       ...
       if (preview['list'] is List) parts = (preview['list'] as List).map((e) => ParticipantPreview.fromJson(e)).toList();
    }
    return Notebook(
      ...
      title: json['title'] ?? '', coverType: json['cover_type'] ?? 'color', color: json['color'], coverImage: json['cover_image'],
      templateType: json['template_type'] ?? 'study', collaborationMode: json['collaboration_mode'] ?? 'study_group',
      isPublished: int.tryParse(json['is_published']?.toString() ?? '0') ?? 0, price: double.tryParse(json['price']?.toString() ?? '0.0') ?? 0.0,
      description: json['description'], authorName: json['author_name'], syncedWithCloud: json['synced_with_cloud'] ?? 1,
      isDeleted: json['deleted_at'] != null ? 1 : 0, updatedAt: upAt ?? TimeService().nowMs(), version: int.tryParse(json['version']?.toString() ?? '1') ?? 1,
      role: json['role'] ?? 'owner', alternativeTitle: json['alternative_title'], sharingType: json['sharing_type'] ?? 'full',
      tags: parseTags(json['tags']), isArchived: json['is_archived'] == 1 || json['is_archived'] == true,
      isFavorite: json['is_favorite'] == 1 || json['is_favorite'] == true, origin: json['origin'],
      participants: parts, participantsTotal: totalParts, onlineCount: oCount, lastUpdatedByName: json['last_updated_by_name'],
  ...
  static List<String> parseTags(dynamic tagsJson) {
    ...
    if (tagsJson is String) return tagsJson.split(',').where((t) => t.isNotEmpty).toList();
    return [];
  }
}
